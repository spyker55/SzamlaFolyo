-- Let an irat be filed under an existing ugy as the next alszam.
--
-- Until now iktat_document() always opened a new ugy with alszam = 1, so the
-- alszam half of the iktatoszam could never reach 2 — the data model carried
-- it, the iktatokonyv ordering sorted by it, but nothing could produce it.
-- That is the whole point of foszam/alszam: a dijbekero and the invoice
-- issued against it are one ugy, not two.
--
-- The argument list changes, so the old function has to go first: CREATE OR
-- REPLACE cannot add a parameter. Callers passing two arguments keep working
-- because p_ugy_id defaults to null, which is the previous behaviour exactly.

drop function if exists public.iktat_document(uuid, jsonb);
...
begin
  ...
  if p_ugy_id is null then
    -- New ugy: gapless foszam allocation under the iktatokonyv row lock.
    ...
    -- Existing ugy: the next alszam, serialized by the ugy row lock the same
    -- way next_foszam is serialized by the iktatokonyv row lock.
    select * into v_ugy
    from public.ugy
    ...
    if not found then
      raise exception 'ugy not found';  -- do not leak existence across tenants
    end if;
    ...
    -- The year comes from the ugy, not from the document: an invoice arriving
    -- in January stays under its 2026 foszam as IKT/4-2/2026.
    v_ev       := v_ugy.ev;

    -- max()+1 rather than a stored counter, so a rolled-back iktatas restores
    -- the number by construction — the same property the foszam gets from
    -- rolling back next_foszam. An ervenytelenitett irat keeps its alszam and
    -- is counted here, because a withdrawn number must never be reissued.
